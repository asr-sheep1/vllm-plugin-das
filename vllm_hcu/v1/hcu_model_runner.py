...
from vllm_hcu.ops.topk_topp_sample import HcuSampler 
...
class GPUModelRunner(
    ...
    def __init__(
        ...
        # Sampler
        self.sampler = HcuSampler(logprobs_mode=self.model_config.logprobs_mode)
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> Callable | None:
        ...
        # Track re-added requests on non-last PP ranks that need token_ids_cpu
        # fix-up after add_request. On non-last ranks, output_token_ids does
        # not include accepted draft tokens, so add_request() can place tokens
        # at stale positions.
        ...
        for i, req_id in enumerate(req_data.req_ids):
            ...
            if req_index is None:
                ...
                if not self.use_async_scheduling and not is_last_rank and new_token_ids:
        ...
        for request in reqs_to_add:
            ...
            if not self.use_async_scheduling:
                ...
                if fix_data is not None:
                    new_toks, num_computed = fix_data
                    start = num_computed
    ...
    def _build_attention_metadata(
        ...
        if ( 
            (not self.enable_lightly_cp) and spec_decode_common_attn_metadata is not None and (
    ...
    def execute_model(
        ...
        with (
            ...
            (attn_metadata, spec_decode_common_attn_metadata, scatter_indexes_tensor, gather_indexes_tensor) = (
    ...
    def _copy_valid_sampled_token_count(
        ...
        self._valid_sampled_token_count_gpu = valid_sampled_tokens_count
